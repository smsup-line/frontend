...
class ApiClient {
  ...
  async request(endpoint, options = {}) {
    ...
    try {
      // ใช้ fetchWithTimeout สำหรับ direct backend calls (ไม่ใช่ Next.js API routes)
      // เพื่อป้องกัน connection timeout errors
      const useTimeout = !isNextApiRoute;
      const response = useTimeout
        ? await fetchWithTimeout(url, config, DEFAULT_TIMEOUT)
      ...
      if (error.name === 'AbortError' || error.name === 'TimeoutError' || error.message.includes('timeout')) {
        console.error('API request timeout:', { url, endpoint, error: error.message });
        throw new Error(`Request timeout: ไม่สามารถเชื่อมต่อ API ได้ภายใน ${DEFAULT_TIMEOUT}ms`);
      }
      
      // Handle connection errors (ETIMEDOUT, ECONNREFUSED, ENOTFOUND)
      if (error.code === 'ETIMEDOUT' || error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
        console.error('API connection error:', { url, endpoint, code: error.code, error: error.message });
        throw new Error(`Connection error: ไม่สามารถเชื่อมต่อ API ได้ (${error.code || 'UNKNOWN'})`);
      }
      
      // ถ้าเป็น network error
      if (error.name === 'TypeError' && error.message.includes('fetch')) {
        console.error('API request failed - Network error:', { url, endpoint, error: error.message });
        throw new Error('ไม่สามารถเชื่อมต่อ API ได้ กรุณาตรวจสอบว่า API server รันอยู่');
      }
      
      console.error('API request failed:', { url, endpoint, error: error.message });
      throw error;
    }
  }
...
export const employeeApi = {
  ...
  getByLineToken: (lineToken, shopId) => {
    const params = new URLSearchParams({ line_token: lineToken });
    if (shopId) params.set('shop_id', shopId);
    return new ApiClient().get(`/api/employeetokenline?${params.toString()}`);
  },
};
...
export const customerTokenLineApi = {
  getByLineToken: (lineToken, shopId) => {
    const params = new URLSearchParams({ line_token: lineToken });
    if (shopId) params.set('shop_id', shopId);
    return new ApiClient().get(`/api/customertokenline?${params.toString()}`);
  },
};
...
export const customerApi = {
  ...
  checkPhoneDuplicate: (phone, storeId, branchId) => {
    ...
    return new ApiClient().get('/api/customers/check-phone', params);
  },
};
...
// Check OTP API
export const checkOtpApi = {
  check: (data) => new ApiClient().post('/api/check-otp', data),
};
...
export const authApi = {
  ...
  lineLogin: (lineToken, name, avatarUrl, shopId, branchId, userType, userId, phone, linkEmployeeId) =>
    new ApiClient().post('/api/auth/line-login', { 
      ...
      link_employee_id: linkEmployeeId || undefined,
    }),
...
export const receiptsApi = {
  ...
  // Customer receipts - get receipts by customer_id
  getCustomerReceipts: (customerId) => new ApiClient().get('/api/receipt-scanner-customer', { customer_id: customerId }),
