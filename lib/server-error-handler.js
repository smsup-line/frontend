...
if (typeof process !== 'undefined' && process.env.NEXT_RUNTIME === 'nodejs') {
  ...
  const BLOCKED_IPS = ['205.185.127.97', '66.96.20.147'];
  ...
  try {
    ...
    net.createConnection = function(options, callback) {
      ...
      // Block specific IPs BEFORE attempting connection
      if (hostname && BLOCKED_IPS.some(ip => hostname.includes(ip) || hostname === ip)) {
        console.warn(`[BLOCKED] net.createConnection PREVENTED: ${hostname}:${port} (blocked IP - connection prevented)`);
        const error = new Error('Connection blocked: blocked IP address');
        error.code = 'ECONNREFUSED';
        error.address = hostname;
        error.port = port;
        // Call callback immediately with error to prevent connection attempt
        if (callback) {
          setImmediate(() => callback(error));
        }
        // Return mock socket that will never connect
        const mockSocket = {
          on: () => mockSocket,
          end: () => mockSocket,
          write: () => false,
          destroy: () => mockSocket,
          connect: () => { setImmediate(() => mockSocket.emit('error', error)); return mockSocket; },
          emit: () => mockSocket,
        };
        setImmediate(() => mockSocket.emit('error', error));
        return mockSocket;
      }
  ...
  if (typeof process.setUncaughtExceptionCaptureCallback === 'function') {
    try {
      ...
      process.setUncaughtExceptionCaptureCallback((error) => {
        // Fast path for blocked IPs - prevent connection attempts
        if (error.code === 'ETIMEDOUT' && error.address) {
          if (BLOCKED_IPS.some(ip => error.address.includes(ip) || error.address === ip) || error.address === '205.185.127.97') {
            console.warn(`[BLOCKED & CAPTURED] ETIMEDOUT to blocked IP: ${error.address}:${error.port || 80} - Connection prevented at capture level`);
            return; // Prevent process crash - don't rethrow
          }
        }
