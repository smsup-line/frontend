...
if (typeof process !== 'undefined' && process.env.NEXT_RUNTIME === 'nodejs') {
  ...
  // Block at net module level (lowest level)
  try {
    ...
    net.createConnection = function(options, callback) {
      ...
      // Block specific IPs
      if (hostname && BLOCKED_IPS.some(ip => hostname.includes(ip))) {
        console.warn(`[BLOCKED] net.createConnection blocked: ${hostname}:${port} (blocked IP)`);
