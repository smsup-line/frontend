...
export function EditAdminSheet({ open, onOpenChange, admin, stores, onSuccess }) {
  ...
  useEffect(() => {
    if (admin && open) {
      form.reset({
        ...
        password: '',
        role: admin.level || admin.role || undefined,
        storeId: admin.shop_id || admin.storeId || '',
      });
    }
  }, [admin, open, form]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await adminApi.update(admin.id, updateData);
      toast.custom(
        (t) => (
          <Alert variant="mono" icon="primary" onClose={() => toast.dismiss(t)}>
            ...
            <AlertTitle>แก้ไขผู้ดูแลระบบสำเร็จ</AlertTitle>
          </Alert>
        ),
        {
          position: 'top-center',
      ...
      toast.error(error.message || 'ไม่สามารถแก้ไขผู้ดูแลระบบได้');
    }
  };
            ...
            <Form {...form}>
              <form
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Password (เว้นว่างไว้ถ้าไม่ต้องการเปลี่ยน)</FormLabel>
                      <FormControl>
                        <Input
                          type="password"
