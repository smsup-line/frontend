...
export function NewReferralHistorySheet({ open, onOpenChange, referrerId, onSuccess }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      loadAdmins();
      if (referrerId) {
        form.setValue('referrer_id', referrerId);
      }
    }
  }, [open, referrerId, form]);
  ...
  // Auto-fill reward_type เมื่อ referrers ถูกโหลดเสร็จและมี referrerId
  useEffect(() => {
    if (open && referrerId && referrers.length > 0 && settingsCenter) {
      const selectedReferrer = referrers.find(r => r.id === referrerId);
  ...
  }, [open, referrerId, referrers, form, settingsCenter]);
  ...
  const onSubmit = async (data) => {
    try {
      setSaving(true);
      await referralHistoryApi.create(data);
      toast.success('เพิ่มประวัติการแนะนำสำเร็จ');
      form.reset({
        referrer_id: referrerId || '',
        referee_id: '',
        reward_type: 'point',
      ...
      console.error('Failed to create referral history:', error);
      toast.error(error.message || 'ไม่สามารถเพิ่มประวัติการแนะนำได้');
    } finally {
      setSaving(false);
    }
  };
