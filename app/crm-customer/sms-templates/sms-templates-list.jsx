...
import { smsTemplatesApi, storeApi } from '@/lib/api';
import { toast } from 'sonner';
import { EditSmsTemplateSheet } from './edit-sms-template-sheet';
...
export default function SmsTemplatesList() {
  ...
  const loadData = async () => {
    try {
      ...
      const data = await smsTemplatesApi.getAll(params);
  ...
  const handleDelete = async (id) => {
    if (!confirm('คุณต้องการลบ SMS Template นี้หรือไม่?')) return;
    
    try {
      await smsTemplatesApi.delete(id);
      toast.success('ลบ SMS Template สำเร็จ');
      loadData();
    } catch (error) {
      console.error('Failed to delete SMS template:', error);
      toast.error('ไม่สามารถลบ SMS Template ได้');
    }
  };
  ...
  const filteredData = useMemo(() => {
    ...
    return filtered;
  }, [searchQuery, storeFilter, smsTemplates, stores, userRole, userStoreId]);
