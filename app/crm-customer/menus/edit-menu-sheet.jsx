...
export function EditMenuSheet({ open, onOpenChange, menu, onSuccess }) {
  const form = useForm({
  ...
  useEffect(() => {
    if (menu && open) {
      form.reset({
        name: menu.name || '',
        menu_key: menu.menu_key || menu.menuKey || '',
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await menusApi.update(menu.id, data);
      toast.custom(
        (t) => (
          <Alert variant="mono" icon="primary" onClose={() => toast.dismiss(t)}>
            ...
            <AlertTitle>แก้ไขเมนูสำเร็จ</AlertTitle>
          </Alert>
        ),
        {
          position: 'top-center',
      ...
      toast.error(error.message || 'ไม่สามารถแก้ไขเมนูได้');
    }
  };
