...
export function EditPackageSheet({ open, onOpenChange, package: pkg, onSuccess }) {
  ...
  useEffect(() => {
    if (pkg && open) {
      const packageMenus = pkg.menus || [];
      const hasAllMenus = packageMenus.length === 0;
      const menuIds = hasAllMenus ? [] : packageMenus.map(m => m.id || m.menu_id);
      
      form.reset({
        name: pkg.name || '',
        duration_days: pkg.duration_days || pkg.durationDays || 30,
        price: pkg.price || 0,
        menu_ids: menuIds,
        select_all_menus: hasAllMenus,
      });
      
      setSelectAllMenus(hasAllMenus);
    }
  }, [pkg, open, form]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await packagesApi.update(pkg.id, updateData);
      toast.custom(
        (t) => (
          <Alert variant="mono" icon="primary" onClose={() => toast.dismiss(t)}>
            ...
            <AlertTitle>แก้ไขแพ็คเกจสำเร็จ</AlertTitle>
          </Alert>
        ),
        {
          position: 'top-center',
      ...
      toast.error(error.message || 'ไม่สามารถแก้ไขแพ็คเกจได้');
    }
  };
