  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { message: data.message || data.error || 'Failed to fetch admin' },
...
export async function PUT(request, { params }) {
  try {
    ...
    if (!response.ok) {
      const data = await response.json().catch(() => ({}));
      return NextResponse.json(
        { message: data.message || data.error || 'Failed to update admin' },
        { status: response.status }
      );
    }
...
export async function DELETE(request, { params }) {
  try {
    ...
    if (!response.ok) {
      const data = await response.json().catch(() => ({}));
      return NextResponse.json(
        { message: data.message || data.error || 'Failed to delete admin' },
        { status: response.status }
      );
    }
